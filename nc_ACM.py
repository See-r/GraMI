...
from utils.input_data import load_ACM_data
...
parser.add_argument('--weight-decay', type=float, default=0.005, help='Weight decay.Default is 0.005')
parser.add_argument('--hidden-dim', type=int, default=32, help='Number of units in fc_layer.')
...
parser.add_argument('--monit', type=int, default=100,
...
parser.add_argument('--task', type=str, default='nc',
                    help='the choise of tasks,pa/ps is prediction of links, fea is prediction of feature, nc is the node classification task.')
parser.add_argument('--mask-rate-link', type=list, default=[0., 0.],
                    help='The rate of mask each relation .')
parser.add_argument('--mask-rate-fea', type=float, default=0.0, help='The rate of mask orig feature')
parser.add_argument('--src-node', type=int, default=0,  # pas
...
parser.add_argument('--loss-lambda1', type=float, default=0.1, help='Coefficient lambda to balance loss.')
parser.add_argument('--loss-lambda2', type=float, default=0.1, help='Coefficient lambda to balance loss.')
...
save_model_path = 'checkpoint/checkpoint_ACM.pt'


def nc_ACM(args):
    adjlists, edge_metapath_indices_list, features_list, adjM, type_mask, labels, train_val_test_idx = load_ACM_data()
    test_idx = train_val_test_idx['test_idx']
    features_list_orig = copy.deepcopy(features_list)
    ...
    for i in range(n_relations):
        ...
        if i == src_node:
            ...
            # features_list[i] = torch.FloatTensor(np.array(features_list[i]))
            features_list[i] = torch.FloatTensor(
                np.eye(features_list[i].shape[0]))
    ...
    all_adj_train, all_train_edges, all_val_edges, all_val_edges_false, all_test_edges, all_test_edges_false, all_adj_val = mask_test_edges(
        all_adj,
        args.mask_rate_link)
    fea_train, train_feas, fea_val, val_feas, fea_test, test_feas = mask_test_feas(
        features_src, args.mask_rate_fea)
    ...
    features_src = sparse_to_tuple(
        sp.csr_matrix(features_src))
    ...
    pos_weight_a = torch.tensor(
        features_src[2][0] * features_src[2][1] - len(features_src[1])) / len(
        features_src[1])
    norm_a = features_src[2][0] * features_src[2][1] / float(
        (features_src[2][0] * features_src[2][1] - len(features_src[1])) * 2)
    all_adj_label = all_adj_train
    for i in range(len(all_adj_label)):
        all_adj_label[i] = torch.FloatTensor(
            all_adj_label[i].toarray())
    features_src_label = torch.FloatTensor(fea_train.toarray())

    model = GraMI(args.dataset, graph, src_node, feats_dim_list, n_nodes,
                  args.edim,
                  num_features_src,
                  args.hidden_dim,
                  args.hidden1, args.hidden2, args.num_heads, args.dropout,
                  encsto=args.encsto, gdc=args.gdc,
                  ndist=args.noise_dist, copyK=args.K, copyJ=args.J,
                  device=args.device)
    ...
    for epoch in range(args.epochs):
        ...
        optimizer.zero_grad()
        label_a, (
            recovered_all, pred_f, fea_recover, mu_list, muf, logvar_list, logvar_f,
            z_all, zf,
            za, z_scaled1_all,
            z_scaled2_all,
            z_scaledf,
        ...
        loss_recover = get_mseloss_score(mask=mask_fea, fea_orig=fea_train,
        ...
        for (hidden_emb1, hidden_emb2, val_edges, val_edges_false) in list(
            ...
            roc_cur, ap_cur = get_roc_score(hidden_emb1, hidden_emb2, val_edges,
                                            val_edges_false, args.gdc)
            ap_val_all.append(ap_cur)
        val_fea_loss = get_maeloss_score(dic=val_feas, fea_orig=fea_val,
                                         fea_pred=fea_recover)
        print(
            "Epoch:", '%04d' % (epoch + 1),
        ...
        if ((epoch + 1) % args.monit == 0):
            ...
            with torch.no_grad():
                label_a, (
                    recovered_all, pred_f, fea_recover, mu_list, muf, logvar_list,
                    logvar_f,
                    z_all, zf, za,
                    z_scaled1_all,
                    z_scaled2_all,
                ...
                for (hidden_emb1, hidden_emb2, test_edges, test_edges_false) in list(
                    ...
                    hidden_emb1 = hidden_emb1.detach().cpu().numpy()
                    hidden_emb2 = hidden_emb2.detach().cpu().numpy()
                    roc_score, ap_score = get_roc_score(hidden_emb1, hidden_emb2,
                                                        test_edges,
                                                        test_edges_false,
                                                        args.gdc)
                    roc_score_all.append(roc_score)
                    ap_score_all.append(ap_score)
                evaluate_results_nc(torch.squeeze(z_all[0])[test_idx].cpu(), labels[test_idx])
