...
parser.add_argument('--num-heads', type=int, default=4, help='Number of the attention heads. Default is 8.')
...
parser.add_argument('--task', type=str, default='ps',
                    help='the choise of tasks,pa/ps is prediction of links, fea is prediction of feature.')
...
if args.task == 'pa':
    args.mask_rate_link = [0.1, 0.]
elif args.task == 'ps':
    args.mask_rate_link = [0., 0.1]
elif args.task == 'fea':
    args.mask_rate_fea = 0.1
    args.loss_lambda1 = 0.1
    args.loss_lambda2 = 0.5
...
def main_ACM(args):
    ...
    for epoch in range(args.epochs):
        ...
        if args.task == 'pa':
            print("link_val_ap=", "{:.5f}".format(ap_val_all[0]))
        elif args.task == 'ps':
            print("link_val_ap=", "{:.5f}".format(ap_val_all[1]))
        elif args.task == 'fea':
            print("fea_val_ap=", "{:.5f}".format(val_fea_loss))
        print(
    ...
    with torch.no_grad():
        label_a, (
            recovered_all, pred_f, fea_recover, mu_list, muf, logvar_list, logvar_f, z_all, zf, za,
            z_scaled1_all,
            z_scaled2_all,
            z_scaledf,
            z_scaled_a, eps_all, epsf, rk_all) = model(
